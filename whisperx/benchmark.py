...
def load_tedlium(subset: str = "test"):
    """Load TEDLIUM Release 3 from Hugging Face."""
    print(f"Loading TEDLIUM dataset ({subset}) from Hugging Face...")
    try:
        dataset = load_dataset("LIUM/tedlium", "release3", split=subset, trust_remote_code=True)
...
def benchmark(dataset, model_size="large-v2", device="cuda", compute_type="float16", batch_size=4, limit=None):
    ...
    n_samples = limit if limit else len(dataset)
    print(f"\nBenchmarking on {n_samples} samples...")
    ...
    for i, item in enumerate(dataset):
        ...
        # HF dataset provides: audio (dict with 'array' and 'sampling_rate'), text, etc.
